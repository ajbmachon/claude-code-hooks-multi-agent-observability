...
PORT="$SERVER_PORT" bun run start &
SERVER_PID=$!
sleep 3
...
FILTERS=$(curl -s "$SERVER_URL/events/filter-options")
if [ $? -eq 0 ]; then
...
echo '{"session_id":"demo-test","tool_name":"Bash","tool_input":{"command":"echo test"}}' | \
    uv run .claude/hooks/send_event.py --source-app demo --event-type PreToolUse --server-url "$SERVER_URL/events"
...
RECENT=$(curl -s "$SERVER_URL/events/recent?limit=5")
if [ $? -eq 0 ]; then
...
echo "1. In terminal 1: cd apps/server && PORT=$SERVER_PORT bun run dev"
echo "2. In terminal 2: cd apps/client && VITE_OBSERVABILITY_API_URL=$SERVER_URL VITE_OBSERVABILITY_WS_URL=ws://localhost:$SERVER_PORT/stream bun run dev"
echo "3. Open the printed Vite URL in your browser"
